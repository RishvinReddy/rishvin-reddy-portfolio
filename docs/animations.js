  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof gsap !== 'undefined') {
      ...
      if (isIndexPage) {
        ...
        // H1 Title 3D Reveal
        heroTl.fromTo("h1", 
          { y: 30, opacity: 0, rotateX: -10, transformPerspective: 800 }, 
          { y: 0, opacity: 1, rotateX: 0, duration: 0.25 }, 0.1
        )
        // Intro Paragraph fade & slide
        .fromTo("h1 + p", 
          { y: 15, opacity: 0 }, 
          { y: 0, opacity: 1, duration: 0.25, ease: "power3.out" }, 0.2
        )
        // Buttons stagger
        .fromTo(".magnetic-btn", 
          { y: 15, opacity: 0, scale: 0.95 }, 
          { y: 0, opacity: 1, scale: 1, duration: 0.25, stagger: 0.1, ease: "back.out(1.5)" }, 0.3
        )
        // Tech Stack Marquee blur reveal
        .fromTo(".marquee-container", 
          { opacity: 0, filter: "blur(5px)" }, 
          { opacity: 0.7, filter: "blur(0px)", duration: 0.25, ease: "power2.out" }, 0.4
        )
        // Profile Image / 3D Tilt Card reveal
        .fromTo(".tilt-card", 
          { scale: 0.9, opacity: 0, rotateY: 20, x: 20, transformPerspective: 1000 }, 
          { scale: 1, opacity: 1, rotateY: 0, x: 0, duration: 0.3, ease: "expo.out" }, 0.2
        );
    ...
    if (typeof gsap !== 'undefined') {
      ...
      magnetics.forEach(btn => {
        ...
        btn.addEventListener('mousemove', (e) => {
          ...
          gsap.to(btn, { x: x, y: y, duration: 0.25, ease: "power3.out" });
          if(btn !== span) gsap.to(span, { x: x*0.5, y: y*0.5, duration: 0.25, ease: "power3.out" });
        });

        btn.addEventListener('mouseleave', () => {
          gsap.to(btn, { x: 0, y: 0, duration: 0.25, ease: "elastic.out(1, 0.3)" });
          if(btn !== span) gsap.to(span, { x: 0, y: 0, duration: 0.25, ease: "elastic.out(1, 0.3)" });
        });
      });
    ...
    tiltCards.forEach(card => {
      card.addEventListener('mousemove', (e) => {
        ...
        if (typeof gsap !== 'undefined') {
          gsap.to(card, { rotationX: rotateX, rotationY: rotateY, duration: 0.25, ease: "power2.out", transformPerspective: 1000 });
        } else {
      ...
      card.addEventListener('mouseleave', () => {
        if (typeof gsap !== 'undefined') {
          gsap.to(card, { rotationX: 0, rotationY: 0, duration: 0.25, ease: "power2.out" });
        } else {
    ...
    function showToast(message, type = 'success') {
      ...
      toast.className = `flex items-center gap-3 px-5 py-3 rounded-full shadow-2xl backdrop-blur-xl border border-white/10 transition-all duration-200 ease-out transform translate-y-8 opacity-0 scale-95
...
document.addEventListener("DOMContentLoaded", () => { 
  // Phase 2: Clip-path intersection observer
  ...
  document.querySelectorAll('.reveal-clip-path').forEach(el => clipObserver.observe(el));
 });
